Ext.define('Locator.controller.App', {
				...
				getPlaces : function(type){
								...
								console.log(Ext.Object.fromQueryString(window.location.search.substring(1)));
								
								// Show the place list page
								me.showPlacesContainer(type);
								
								if(type === me.activeCategoryType){
												return;
								}
									
								// If the type is not a valid type, ignore it
								if(Ext.getStore('Categories').findRecord('type', type)){
				 ...
				 */
				showPlacesContainer : function(type, reverse){
								var me = this,
				...
				showPlacesMarkers : function(extMap, gMap){
								...
								userLocation = me.util.defaultLocation.split(','),
								...
								*/
								google.maps.event.addDomListener(ib.bubble_, 'click', function () {
												if(me.activeCategoryType){
																me.redirectTo('categories/' + me.activeCategoryType + '/' + ib.placeId);
												}
								});
